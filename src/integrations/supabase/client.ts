...
// Create a more robust session refresh mechanism
const MAX_RETRIES = 3;
const RETRY_DELAY = 1000; // 1 second

// Helper to handle auth errors in database operations with retry logic
const handleAuthError = async (error: any, retryCount = 0): Promise<boolean> => {
  if (error?.message?.includes('JWT expired') || 
    ...
    console.log(`Authentication error detected (attempt ${retryCount + 1}/${MAX_RETRIES}), attempting to refresh session`);
    
    if (retryCount >= MAX_RETRIES) {
      console.error('Maximum retry attempts reached for session refresh');
      return false;
    }
    ...
    try {
      ...
      if (refreshError) {
        ...
        return new Promise(resolve => {
          setTimeout(async () => {
            const result = await handleAuthError(error, retryCount + 1);
            resolve(result);
          }, delay);
        });
      }
...
export const checkUserSubscription = async (userId: string) => {
  ...
  try {
    ...
    if (error) {
      ...
      if (refreshed) {
        return checkUserSubscription(userId); // Retry after refresh
      }
...
export const updateUserProfile = async (userId: string, cpf: string) => {
  try {
    ...
    if (error) {
      const refreshed = await handleAuthError(error);
      if (refreshed) {
        return updateUserProfile(userId, cpf); // Retry after refresh
      }
...
export const removeDuplicateTransactions = async () => {
  try {
    ...
    if (error) {
      const refreshed = await handleAuthError(error);
      if (refreshed) {
        return removeDuplicateTransactions(); // Retry after refresh
      }
    ...
    if (duplicates.length > 0) {
      ...
      if (deleteError) {
        const refreshed = await handleAuthError(deleteError);
        if (refreshed && duplicates.length > 0) {
          // Retry the delete operation after refresh
          ...
          if (retryDelete.error) {
            console.error("Error deleting duplicates after refresh:", retryDelete.error);
            return { removed: 0 };
          }
        } else {
...
export const createStripeCheckout = async (priceId: string) => {
  try {
    ...
    if (error) {
      const refreshed = await handleAuthError(error);
      if (refreshed) {
        return createStripeCheckout(priceId); // Retry after refresh
      }
...
export const getUserSubscription = async () => {
  try {
    ...
    if (error) {
      const refreshed = await handleAuthError(error);
      if (refreshed) {
        return getUserSubscription(); // Retry after refresh
      }
...
export const deleteJournalEntry = async (entryId: string) => {
  try {
    ...
    if (userError) {
      const refreshed = await handleAuthError(userError);
      if (refreshed) {
        return deleteJournalEntry(entryId); // Retry after refresh
      }
    ...
    if (verifyError) {
      const refreshed = await handleAuthError(verifyError);
      if (refreshed) {
        return deleteJournalEntry(entryId); // Retry after refresh
      }
    ...
    if (count === 0) {
      ...
      if (directError) {
        const refreshed = await handleAuthError(directError);
        if (refreshed) {
          // Retry the RPC call after refresh
          const retryRpc = await supabase.rpc('delete_journal_entry', { 
            entry_id_param: entryId,
            user_id_param: user.id 
          });
          
          if (retryRpc.error) {
            console.error('Error in direct deletion via RPC after refresh:', retryRpc.error);
            throw retryRpc.error;
          }
          
          return { success: !!retryRpc.data, count: retryRpc.data ? 1 : 0 };
        }
...
export const fetchJournalEntries = async () => {
  try {
    ...
    if (userError) {
      const refreshed = await handleAuthError(userError);
      if (refreshed) {
        return fetchJournalEntries(); // Retry after refresh
      }
      console.error('Error getting user:', userError);
      throw userError;
    }
...
export const updateChallengeRankings = async (challengeId: string) => {
  try {
    ...
    if (recordsError) {
      const refreshed = await handleAuthError(recordsError);
      if (refreshed) {
        return updateChallengeRankings(challengeId); // Retry after refresh
      }
    ...
    if (participantsError) {
      const refreshed = await handleAuthError(participantsError);
      if (refreshed) {
        return updateChallengeRankings(challengeId); // Retry after refresh
      }
