...
export function Achievements({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('running_records').select('distance').eq('user_id', session.user.id);
      if (error) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('running_challenges').select('*').order('created_at', {
        ascending: false
      }).limit(1);
      if (error) {
  ...
  return <Card className="border-none bg-gradient-to-br from-amber-50 to-orange-50 overflow-hidden shadow-md transition-all duration-300 hover:shadow-lg group">
      ...
      <CardContent>
        <div className="flex flex-col">
          ...
          achievements.slice(0, 3).map((achievement, index) => <div key={index} className="w-10 h-10 rounded-full flex items-center justify-center" style={{
          ...
          {latestChallenge ? <div className="bg-amber-100/70 rounded-lg p-3">
            ...
            </div> : <div className="bg-amber-100/70 rounded-lg p-3">
          ...
          {achievements.length > 0 && <div className="mt-3 pt-3 border-t border-amber-200">
              <div className="text-sm font-medium text-amber-800 mb-2">Suas Conquistas Recentes</div>
              <div className="space-y-2">
                {achievements.slice(0, 2).map((achievement, index) => <div key={index} className="flex items-center gap-2 bg-white/60 p-2 rounded-lg">
